...
sub kill_node_daemon {
    for ($i = 0; $i < $nr_daemon; ++$i) {
        ...
        $d_param = "-x -p $port -N -n $nodename=$iorfile --deployment-nc ". $ENV{'NameServiceIOR'};
        $daemon_stopper = $tg_daemons[$i]->CreateProcess ($d_cmd, $d_param);
        $daemon_stopper->SpawnWaitKill ($PerlACE::Process::WAIT_DELAY_FACTOR * $tg_daemons[$i]->ProcessStartWaitInterval ());
        $DEAMONS[$i]->WaitKill ($PerlACE::Process::WAIT_DELAY_FACTOR * 5);
    }
}
...
if ($pl_status != 0) {
    ...
    $EM->WaitKill ($tg_exe_man->ProcessStopWaitInterval ());
}
